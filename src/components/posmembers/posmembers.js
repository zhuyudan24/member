...
export default {
  ...
  data() {
    return {
      ...
      toggleTag: false,
      sceneValue: "member",
      navpath: [{ name: "首页", path: "" }, { name: "POS会员", path: "" }],
      memberSearchStr: -1,
      tableData: [],
      base: false,
      pageName: "posMember",
      ...
      rules: {
        ...
        settlementStoreId: [
          { required: true, message: "请输入成本主体", trigger: "change" }
        ]
      },
  ...
  watch: {
    ...
    phoneNameCard(newval) {
      sessionStorage.setItem("posPhoneNameCard", newval);
    }
  },
  activated() {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.page.pageSize = 20;
      sessionStorage.setItem("poscurrentPage", val);
      this.getAjaxMembers();
    },
            ...
            this.memberData = res.data.result.page.result.map(ele => ({
              ...
              lastCostTime:
                ele.lastCostTime != -1
                  ...
                  : "--", // 最近消费时间
              createTime:
                ele.createTime != -1 ? formatLongTime(ele.createTime, 1) : "--", // 新增时间
              attentionDate:
                ele.attentionDate != -1
                  ? formatLongTime(ele.attentionDate, 1)
                  : "--", // 关注时间
              cancelAttentionTime:
              ...
              attentionTime:
                ele.attentionTime != -1
                  ? formatLongTime(ele.attentionTime, 1)
                  : "--", // 关注时间
              crateCardDateString:
  ...
  created() {
    ...
    this.getProcessList();
    this.phoneNameCard = !!sessionStorage.getItem("posphoneNameCard")
      ? sessionStorage.getItem("posphoneNameCard")
      : "";
    this.page.currentPage = !!sessionStorage.getItem("poscurrentPage")
