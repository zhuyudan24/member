    ...
    <div class="gic-goods-selector" v-show="complexData.length == 0">
      <div class="good-container" v-for="(condition, index) in conditions" :key="index">
        ...
        <ul class="goods-lists">
          <li class="good-type"
              v-for="(good, inx) in condition.goods"
              :class="[condition.goodsList.length == 5 ? 'good-forbid': '']"
              :key="inx + 1" 
              @click="addGoodsCondition(good, index)">
      ...
      list.forEach(el => {
        ...
        if (el.goodsList && el.goodsList.length) {
          el.goodsList.forEach(item => {
            ...
            list.operate = item.condition == null ? '' : item.condition == 1 ? 'intersect' : item.condition == 1 ? 'union' : 'diff'; // 条件
            // 品类
...
.gic-goods-selector {
  ...
  .goods-condition {
    .goods-box {
      ...
      border: 1px solid rgba(266, 244, 244, .9);
      &:hover {
        border-color: #1890ff;
      }  
    }  
